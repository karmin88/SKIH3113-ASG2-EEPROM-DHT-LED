...
#define LED_PIN D1      // Define the LED pin
...
void setup() {
  ...
  digitalWrite(LED_PIN, ledState ? HIGH : LOW);  // Apply initial LED state
}
...
void createWebServer(int webtype) {
  server.on("/", [webtype]() {
    ...
    if (webtype == 0) {  // Client mode
      ...
      content += "<tr><td>LED State</td><td>" + String(ledState ? "On" : "Off") + "</td></tr></table>";
    } else {  // AP mode
      ...
      content += "<tr><td>LED State</td><td>";
      content += "<input type='radio' name='ledState' value='on'" + String(ledState ? " checked" : "") + "> On";
      content += "<input type='radio' name='ledState' value='off'" + String(!ledState ? " checked" : "") + "> Off";
      content += "</td></tr>";
  ...
  server.on("/setting", []() {
    ...
    bool newLedState = server.arg("ledState") == "on";  // Retrieve new LED state from the form
    ...
    writeData(ssid, password, deviceID, ledState, tempOld, humOld);
...
void writeData(String a, String b, String c, bool d, float e, float f) {
  ...
  EEPROM.write(60, d);  // Write LED state to EEPROM
...
void readData() {
  ...
  ledState = EEPROM.read(60);  // Read LED state from EEPROM
  ...
  Serial.println("LED state from EEPROM: " + String(ledState ? "On" : "Off"));
